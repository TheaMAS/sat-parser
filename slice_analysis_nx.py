...
def get_graph_slice_at(graph, time):
    ...
    static (networkx) graph `g` that exists at that point in time.
    """
...
def get_diameter(g):
    """
    Given a static (networkx) graph `g`, we return the diameter of
    that graph. If the graph is not connected, we return `-1`.
...
def get_vertex_count(g):
    """
    Returns the number of vertices in a (networx) graph object.
    """
    return g.number_of_nodes()

def get_edge_count(g):
    """
    Returns the number of edges in a (networkx) graph object.
    """
    return g.number_of_edges()

def get_degree_distribution(g):
    """
    Returns the degree distribution of g.

    See `https://en.wikipedia.org/wiki/Degree_distribution` for details.
    """
    hist = nx.degree_histogram(g)

    # for v in g.nodes():
    #     print("--{} has degree {}".format(v, g.degree[v]))
...
# print(iterate_slices(graph, critical_times, [get_diameter, get_edge_count]))
# exit()
